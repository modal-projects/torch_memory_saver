...
class TorchMemorySaver:
    ...
    def region(
        ...
        cpu_backup_backend selects the host shadow used when enable_cpu_backup
        is True: "mmap" (default on CUDA; reclaimable RSS) or "pinned"
        (cudaMallocHost/hipHostMalloc; faster copies). ROCm is pinned-only
        (mmap is rejected). On CUDA, host shadows are released on resume;
        get_cpu_backup is only valid while paused.
        """
    ...
    def _cpu_backup_backend(
        ...
        # Shared by region() and cuda_graph() so validation/defaults stay in one place.
        # Always run (no ternary at call sites) so backend-without-enable raises.
        ...
        if cpu_backup_backend is None:
            ...
            # Empty string is treated as unset (same as C++).
            if env:
                cpu_backup_backend = env  # type: ignore[assignment]
            else:
                # ROCm stays pinned-only until mmap reclaim is validated on AMD.
                cpu_backup_backend = "pinned" if torch.version.hip else "mmap"
...
class _TorchMemorySaverImpl:
    ...
    def _with_region_config(
        ...
        try:
            ...
            if cpu_backup_backend is not None:
                assert cdll.tms_get_cpu_backup_backend().decode("utf-8") == cpu_backup_backend
